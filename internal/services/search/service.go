...
func (s *search) RecursiveSearch(sortField, search, gameID, collectionID string) (*entity.RecursiveSearchItems, *network.Meta, error) {
	...
	if gameID == "" {
		...
		foundGames, _, err := s.serviceGame.List(sortField, search)
		if err != nil {
			return nil, nil, err
		}
	...
	if gameID == "" {
		allGames, _, err = s.serviceGame.List(sortField, "")
		if err != nil {
			return nil, nil, err
		}
	} else {
	...
	for _, game := range allGames {
		...
		for _, collection := range allCollections {
			...
			foundDecks, _, err := s.serviceDeck.List(game.ID, collection.ID, sortField, search)
			if err != nil {
				return nil, nil, err
			}
			...
			allDecks, _, err := s.serviceDeck.List(game.ID, collection.ID, sortField, "")
			if err != nil {
				return nil, nil, err
			}
